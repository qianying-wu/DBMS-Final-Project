...
(function(){
  ...
  // 取得 JWT 憑證 (假設你登入後存在 localStorage 或 cookie)
  ...
  async function loadProfilesFromDB(){
    try {
      const res = await fetch('/api/pv/savePV', { // 請替換成實際的後端路由
  ...
  async function saveProfileToDB(profileData){
    try {
      const res = await fetch('/api/pv/savePV', { // 對應你的路由 /savePV
        method: 'POST',
        headers: {
      ...
      return result; // 預期後端回傳最新的 profile 物件（包含 ID）
    } catch (err) {
      console.error(err);
      alert('儲存到資料庫失敗');
      throw err;
  ...
  // 補齊資料欄位（改為對記憶體中的 cachedProfiles 做整理）
  function normalizeProfiles(profiles){
    return profiles.map((profile, index) => ({
      ...profile,
      id: profile.id || profile._id || Date.now() + index, // 相容 MongoDB 的 _id
      name: profile.name || profile.data?.name || `履歷 ${index + 1}`,
  ...
  // 以下通知功能是備援
  function loadNotifications(){ return JSON.parse(localStorage.getItem('notifications')||'[]'); }
  ...
  function renderResumeGallery(){
    const ps = normalizeProfiles(cachedProfiles);
    resumeGallery.innerHTML = '';
    const activeId = getActiveProfileId();
    ps.forEach(p => {
      const card = document.createElement('article');
      ...
      title.addEventListener('blur', async () => {
        ...
        const idx = cachedProfiles.findIndex(item => String(item.id || item._id) === String(p.id));
        if (idx >= 0 && cachedProfiles[idx].name !== newName) {
          cachedProfiles[idx].name = newName;
          cachedProfiles[idx].updatedAt = new Date().toISOString();
          await saveProfileToDB(cachedProfiles[idx]);
          if (String(p.id) === String(getActiveProfileId())) editorTitle.textContent = newName;
  ...
  function loadEditorData(){
    const activeId = getActiveProfileId();
    const profile = cachedProfiles.find(x=>String(x.id || x._id)===String(activeId)) || cachedProfiles[0] || null;
    const data = profile ? profile.data : {};
    if (profile && !activeId) setActiveProfileId(profile.id || profile._id);
    editorTitle.textContent = profile?.name || '新增履歷';
    $('name').value = data.name||'';
    $('school').value = data.school||'';
    $('grade').value = data.grade||'';
  ...
  async function createResume(){
    const now = new Date().toISOString();
    const newProfile = { name: '新履歷', createdAt: now, updatedAt: now, data: {} };
    try {
      const savedProfile = await saveProfileToDB(newProfile);
      const targetId = savedProfile.id || savedProfile._id || Date.now();
  ...
  delResume.addEventListener('click', async ()=>{
    const activeId = getActiveProfileId(); if(!activeId){ alert('沒有選中的履歷'); return; }
    if(!confirm('確定要刪除此履歷嗎？')) return;
    
    const success = await deleteProfileFromDB(activeId);
    if (success) {
      await loadProfilesFromDB();
      if (cachedProfiles.length) setActiveProfileId(cachedProfiles[0].id || cachedProfiles[0]._id); 
  ...
  saveBtn.addEventListener('click', async ()=>{
    ...
    const currentProfile = cachedProfiles.find(p => String(p.id || p._id) === String(activeId));
    
    const profilePayload = {
      id: activeId,
      name: data.name || currentProfile?.name || '履歷',
      updatedAt: new Date().toISOString(),
      data: data
    };

    try {
      const serverProfile = await saveProfileToDB(profilePayload);
      if (serverProfile && (serverProfile.id || serverProfile._id)) {
        setActiveProfileId(serverProfile.id || serverProfile._id);
      }
      await loadProfilesFromDB();
      renderResumeGallery(); 
      alert('已成功儲存至資料庫！');
    } catch(e) {
      // 錯誤訊息由 saveProfileToDB 處理
    }
  });
  ...
  $('name').addEventListener('input', (e)=>{
    ...
    const activeId = getActiveProfileId();
    if (!activeId) return;
    const idx = cachedProfiles.findIndex(p=>String(p.id || p._id)===String(activeId));
    if (idx>=0){ 
      cachedProfiles[idx].data = { ...(cachedProfiles[idx].data || {}), name: v }; 
      editorTitle.textContent = v || '未命名履歷';
    }
  });
