...
export function renderContestOverview(contests, teams, selectedContest, contestFavs) {
  ...
  contestsGrid.innerHTML = `${displayContests.map(contest => {
    ...
    const contestTags = realTag
      ? realTag.split(',').map(t => t.trim()).slice(0, 3)
      : (window.AppPreferences ? window.AppPreferences.inferContestTags(contest).slice(0, 3) : []);
    ...
    return `
      <article class="contest-card ${Number(selectedContest) === Number(contest.id) ? 'active' : ''}" data-cid="${contest.id}">
        ...
        <div class="tag-row">
          ${contestTags.map(key => {
      // 如果是資料庫來的標籤字串，就直接顯示；如果是原本舊機制的 key，才呼叫 labelFor 轉換
      const label = (window.AppPreferences && window.AppPreferences.labelFor) ? (window.AppPreferences.labelFor(key) || key) : key;
      return `<span class="match-tag">${escapeHtml(label)}</span>`;
    }).join('')}
        </div>
