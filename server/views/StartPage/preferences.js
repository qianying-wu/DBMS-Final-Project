...
(function(){
  ...
  // 🛠️ 新增安全輔助函式：取得有效的 Token Header（若你們後端是用 Bearer 格式，請將 JWT 改為 Bearer）
  function getAuthHeaders() {
    const token = localStorage.getItem('token');
    return token ? { 'Authorization': `JWT ${token}` } : {};
  }
  ...
  async function loadTags(){
    try {
      const resp = await fetch('/api/auth/preference-tags', {
        headers: { ...getAuthHeaders() } // 🛠️ 補上 Token
      });
      
      // 🛠️ 先確認後端成功回應，才解析 JSON
      if (!resp.ok) throw new Error(`HTTP 錯誤碼: ${resp.status}`);
    ...
    try {
      const resp = await fetch(`/users/${encodeURIComponent(userId)}/preferences`, {
        ...
        headers: { 
          ...
          ...getAuthHeaders() // 🛠️ 補上 Token
        },
        body: JSON.stringify({ preferences })
      });
      
      // 🛠️ 安全解析：只有在 ok 時才回傳成功 json，否則拋出錯誤走 catch
      if (!resp.ok) throw new Error(`HTTP 錯誤碼: ${resp.status}`);
